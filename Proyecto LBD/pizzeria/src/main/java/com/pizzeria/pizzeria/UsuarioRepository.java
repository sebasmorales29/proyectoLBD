...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    @Query(value = "{CALL CRUD.ACTUALIZAR_USUARIO(:id_usuario, :nombre, :prim_apellido, :seg_apellido, :correo, :id_rol, :rol, :activo)}", nativeQuery = true)
    void editarUsuario(@Param("id_usuario") Integer id_usuario,
            @Param("nombre_usuario") String nombr_usuario,
            @Param("nombre") String nombre,
            @Param("prim_apellido") String prim_apellido,
            @Param("seg_apellido") String seg_apellido,
            @Param("correo") String correo,
